...
(function () {
    ...
    plus.page.prototype = {
        ...
        prepare: function (opt) {
            ...
            if ($('.div-form').size() == 1 && $('.search-bar').size()) {
            ...
            function constructGrid(el/*, columns*/) {
                ...
                var grid = $('<div class="div-grid grid"></div>'), origin = el.find('grid').after(grid).remove(), load = origin.attr('noload') === "false", fn = window[origin.attr('onItemClick')],
                    path = origin.attr('path') ? origin.attr('path').split(".") : origin.attr('id').substring(2).split("_"), noHeader = !!origin.attr('noHeader'), isSP = el.attr('commandType') == "sp";
                ...
                origin.find('column').each(function () {
                    ...
                    indexes[col] = index - 0;
                    //columns[index - 0] = col;
                    //if ($(this).hasClass('checkBox')) templ += '<td><input type="checkbox" disabled checked="{' + col + '}" /></td>';

                    if ($(this).hasClass('checkBox')) templ += '<td><img style="width:20px; float:none;" src="../../skin/icon/checked_{' + col + '}-32.png" /></td>';
